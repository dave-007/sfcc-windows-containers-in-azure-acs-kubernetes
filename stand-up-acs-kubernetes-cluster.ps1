...
az group create --name rg-acs-codecamp --location eastus2
...
$windowsPassword = '$UP3RS3CR3+4321' #be smart and change this password
...
#validate!
az acs create --name $ACSClusterName --resource-group $newGroup.name --orchestrator-type kubernetes --windows --admin-password $windowsPassword --agent-count 1 --service-principal $newServiceAccount.appId --client-secret $newServiceAccount.password --ssh-key-value $pathToSSHPublicKeyFile --validate --debug
